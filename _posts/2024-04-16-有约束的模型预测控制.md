...
$$\begin{aligned}&x(1|k) =Ax(0|k)+Bu(0|k) \\
&x(2|k) =Ax(1|k)+Bu(1|k)=A[Ax(0 | k)+Bu(0 | k)]+B(1 | k) \\
&=A^{2}x(0 | k)+ABu(0 | k)+Bu(1 | k) \\
&\cdots \\
&x(i|k) =Ax(i-1 | k)+Bu(i-1 | k)=\cdots  \\
&=A^{i}x(0 | k)+A^{i-1}Bu(0 | k)+A^{i-2}Bu(1 | k)+\cdots+Bu(i-1|k)\end{aligned}$$
...
只取第一个$U$, $$u^{*}(k)=- [I_{p\times p} 0 \cdots 0](\Phi^{T}\mathcal{Q}\Phi+\mathcal{R})^{-1}\Phi^{T}\mathcal{Q}Fx(k)=-Kx(k)$$
